...
assert c.format(123456792123456789) == "T+204128w[s] 2d[S] 1h[] 22m[eS] 3s[Es] 456ms[ES] 789"
assert c.format_datetime(datetime.datetime(2022, 12, 1),
                         datetime.datetime(2023, 1, 15)) == "T+6w[s] 3d[S]"
assert c.format_datetime(datetime.datetime(2023, 1, 15),
                         datetime.datetime(2022, 12, 1)) == "T-6w[s] 3d[S]"
assert c.format_weeks(1.56671) == "T+1w[] 3d[S] 23h[es] 12m[eS] 26s[Es] 208ms[ES]"
assert c.format_days(-1) == "T-1d[]"
assert c.format_hours(241.77) == "T+1w[] 3d[S] 1h[] 46m[eS] 12s[Es]"
...
assert c.format_time(weeks=1, days=2, hours=-1, minutes=1, seconds=1, milliseconds=1,
                     microseconds=1) == "T+1w[] 1d[] 23h[es] 1m[] 1s[] 1ms[] 1"
